...
| MCPツール | 内容 |
| --- | --- |
| `get_photos` | 未取得の写真を受信順に返し、返した分を受信箱から消す。1回に返すのはbase64の合計が上限（既定8MB）までで、残りの枚数も返す。保存したファイルのパスも返す。`pathsOnly: true` では画像を返さず、未取得の写真をすべて取り出してパスだけを返す（画像を見ずにファイルとして別の担当者へ渡す場合に使う） |
