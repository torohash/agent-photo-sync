...
- このPCはufwが有効で受信を既定で拒否しており、mDNS（UDP 5353）と受信ポート（47800〜47819番）の許可が必要。

受信ポートを47800〜47819番に固定した後も、`npm test`（5件。使用中のポートを飛ばすこと、`PHOTOSYNC_PORT_RANGE` の解釈を追加）と `npm run test:e2e` の通過、MCPサーバーが47800番で待ち受けることを確認しました。
